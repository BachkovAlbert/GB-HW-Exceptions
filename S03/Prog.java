...
public class Prog {
    static class Person {
        ...
        public String convertToString() {
            String userDataStr = this.surname + " | " + this.name + " | " + this.patronymic + " | " + this.birthday
    ...
    public static void newUserInfo() {
        ...
        dataCheck(newUser);
        Person user = new Person(newUser.get(0), newUser.get(1), newUser.get(2), newUser.get(3),
                Long.parseLong(newUser.get(4)), newUser.get(5).charAt(0));
    ...
    public static ArrayList<String> dataInput() {
        ...
        System.out.print(
                "Введите данные через пробел в формате: Фамилия Имя Отчество ДатаРождения(дд.мм.гггг) НомерТелефона(71234567890) Пол(f|m): ");
    ...
    public static void dataCheck(ArrayList<String> inputData) {
        try {
            ...
            char sex = inputData.get(5).toLowerCase().charAt(0);

            if (inputBirthday.length() != 10) {
                throw new Exception(
                        "Неверно указана дата рождения. Формат должен быть в виде ДД.ММ.ГГГГ. Повторите ввод.");
            }
